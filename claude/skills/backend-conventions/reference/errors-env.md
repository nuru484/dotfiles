...
- converts Prisma errors first (`handlePrismaError`),
...
Always wrap handlers so thrown errors reach it:
...
Provide a shared `paginate()` helper so every list endpoint produces the same
`meta`. Use `HTTP_STATUS_CODES` constants, not magic numbers.
