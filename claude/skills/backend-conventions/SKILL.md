...
- **Security** (authz hardening, input sanitization, secrets, headers) → the security skill.
- **Tests** (writing/running tests, TDD) → the testing skill.
- **Code review / PR analysis** → the review skill.
...
[ ] Service return types use Prisma.<Model>GetPayload<typeof include>
...
- Per-feature `index.ts` barrels re-export the public surface.
...
- Return `Prisma.<Model>GetPayload<typeof include>`; define a shared `const include`.
