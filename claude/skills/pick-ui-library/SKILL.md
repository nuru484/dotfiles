...
## House-stack overrides (read first)

In this user's house-stack projects (see `frontend-conventions`), three rows
of the list are already decided differently and must NOT be churned:

- **UI primitives**: shadcn/ui (on Radix) is the house component layer; do
  not migrate to base-ui or mix the two primitive systems in one app.
- **State management**: Redux Toolkit + RTK Query is the house state/data
  layer, not zustand.
- **className composition**: the house `cn()` helper (clsx + tailwind-merge)
  already covers the clsx row; cva stays for variants.

Everything else on the list (Sonner, cmdk, motion, NumberFlow, dnd kit,
Virtuoso, recharts, input-otp, next-themes, shiki, Satori, Cobe) is adopted
as-is. Outside house-stack projects the full list applies unchanged.
